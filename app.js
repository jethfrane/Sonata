    ...
    (() => {
      ...
      DEFAULT_SETTINGS = { theme: "light", accentTheme: "blue", uiFontFamily: "system", chartFontFamily: "mono", appFontSize: 15, editorFontSize: 15, previewFontSize: 14, editorLineHeight: 1.6, presentationFontSize: 42, metronomeVolume: 0.55, presentationOrientation: "auto", metronomeBpm: 90, metronomeBeats: 4, uiSounds: true, haptics: true, autoScrollSpeed: 30 };
...
`;
      ...
      const GoogleDriveSync = {
        ...
        userEmail: null, resolveSignIn: null,
      ...
      const StateManager = {
        ...
        createSong() { const now = Util.now(); const s = { id: Util.uid(), title: "", artist: "", description: "", links: [], creator: "", body: "", savedSnapshot: "", manualKey: "auto", isFavorite: false, createdAt: now, updatedAt: now, lastOpenedAt: now }; this.state.songs.unshift(s); this.state.activeId = s.id; this.state.transposeDelta = 0; this.state.capo = 0; this.state.sharedSong = null; this.exitSetlist(); this.saveNow("New song created"); return s; },
      ...
      const InstrumentManager = {
        init() { this.renderCircle(); this.renderPiano(); this.renderFretboard('guitar'); this.bind(); },
        bind() {
        ...
        renderPiano() {
          ...
          for (let octave = 3; octave <= 5; octave++) {
            for (let i = 0; i < 12; i++) {
              ...
              key.className = `piano-key ${isBlack ? 'piano-key-black' : 'piano-key-white'}`;
      ...
      const ExportManager = {
        ...
        renderPng(payload) {
          const columns = parseInt(UIManager.dom.exportColumns.value, 10) || 1; const orientation = UIManager.dom.exportOrientation.value; const scale = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
          const style = getComputedStyle(document.documentElement); const bgColor = style.getPropertyValue('--bg').trim() || "#f6f7f9"; const surfaceColor = style.getPropertyValue('--surface').trim() || "#ffffff"; const textColor = style.getPropertyValue('--text').trim() || "#16181d"; const mutedColor = style.getPropertyValue('--muted').trim() || "#667085"; const lineColor = style.getPropertyValue('--line').trim() || "#d8dde6"; const accentColor = style.getPropertyValue('--accent').trim() || "#1967d2"; const fontFamily = style.getPropertyValue('--chart-font').trim() || "monospace"; const uiFont = style.getPropertyValue('--ui-font').trim() || "sans-serif";
          const textSize = parseInt(StateManager.state.settings.editorFontSize, 10) || 14; const lineHeight = textSize * 1.6; const padding = 48; const hasArtist = Boolean(payload.artist); const headerHeight = hasArtist ? 144 : 120; const footerHeight = 60;
          const canvas = document.createElement("canvas"); const ctx = canvas.getContext("2d"); ctx.font = `${textSize}px ${fontFamily}`;

          let wrappedLines = []; let originIndexMap = [];
          payload.body.split('\n').forEach((line, origIdx) => { const limits = orientation === 'landscape' ? { 1: 120, 2: 55, 3: 35 } : { 1: 90, 2: 42, 3: 26 }; const limit = limits[columns] || limits[1]; if (!line) { wrappedLines.push(""); originIndexMap.push(origIdx); return; } let rem = line; while (rem.length > 0) { wrappedLines.push(rem.substring(0, limit)); originIndexMap.push(origIdx); rem = rem.substring(limit); } });

          let maxLineWidth = 0; wrappedLines.forEach(l => { maxLineWidth = Math.max(maxLineWidth, ctx.measureText(l || " ").width); });
          const columnWidth = Math.max(400, maxLineWidth + 40); const linesPerColumn = Math.ceil(wrappedLines.length / columns);
          const width = (padding * 2) + (columnWidth * columns) - (columns > 1 ? 20 : 0); const height = headerHeight + footerHeight + (linesPerColumn * lineHeight) + padding;
          canvas.width = width * scale; canvas.height = height * scale; ctx.scale(scale, scale);
          ctx.fillStyle = bgColor; ctx.fillRect(0, 0, width, height); ctx.fillStyle = surfaceColor; ctx.fillRect(0, 0, width, headerHeight); ctx.fillStyle = lineColor; ctx.fillRect(0, headerHeight, width, 1);
          
          ctx.fillStyle = accentColor; ctx.beginPath(); if(ctx.roundRect) { ctx.roundRect(padding, 38, 36, 36, 8); } else { ctx.rect(padding, 38, 36, 36); } ctx.fill();
          ctx.fillStyle = "#ffffff"; ctx.font = `800 22px ${uiFont}`; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText("S", padding + 18, 38 + 19);
          ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";

          ctx.fillStyle = textColor; ctx.font = `700 32px ${uiFont}`; ctx.fillText(payload.title, padding + 52, 65); let yOffset = 95; if (hasArtist) { ctx.font = `600 16px ${uiFont}`; ctx.fillStyle = mutedColor; ctx.fillText(`By ${payload.artist}`, padding, yOffset); yOffset += 24; }
          ctx.fillStyle = mutedColor; ctx.font = `600 14px ${uiFont}`; ctx.fillText(`Key: ${payload.keyInfo}   •   BPM: ${payload.bpm}   •   Layout: ${payload.modeName}`, padding, yOffset);

          wrappedLines.forEach((line, i) => {
            const x = padding + (Math.floor(i / linesPerColumn) * columnWidth); const y = headerHeight + padding + ((i % linesPerColumn) * lineHeight) + (textSize * 0.8);
            if (line.startsWith('---')) { ctx.beginPath(); ctx.moveTo(x, y - (lineHeight / 2)); ctx.lineTo(x + columnWidth - 20, y - (lineHeight / 2)); ctx.strokeStyle = lineColor; ctx.stroke(); }
            else if (line.startsWith('# ')) { ctx.fillStyle = accentColor; ctx.font = `bold ${textSize * 1.3}px ${uiFont}`; ctx.fillText(line.substring(2), x, y); ctx.font = `${textSize}px ${fontFamily}`; ctx.fillStyle = textColor; }
            else if (line.startsWith('## ')) { ctx.fillStyle = textColor; ctx.font = `bold ${textSize * 1.1}px ${uiFont}`; ctx.fillText(line.substring(3), x, y); ctx.font = `${textSize}px ${fontFamily}`; ctx.fillStyle = textColor; }
            else if (line.startsWith('> ') || line.startsWith('// ')) { ctx.fillStyle = mutedColor; ctx.font = `italic ${textSize * 0.9}px ${uiFont}`; ctx.fillText(line.substring(2), x, y); ctx.font = `${textSize}px ${fontFamily}`; ctx.fillStyle = textColor; }
            else if (payload.chordLineIndices.has(originIndexMap[i]) && StateManager.state.previewMode !== 'lyrics') {
              ctx.fillStyle = accentColor; ctx.font = `bold ${textSize}px ${fontFamily}`; ctx.fillText(line || "", x, y); ctx.font = `${textSize}px ${fontFamily}`; ctx.fillStyle = textColor;
            }
              ...
              parts.forEach(part => {
                ...
                if (part.startsWith('[') && part.endsWith(']')) {
                  ctx.fillStyle = accentColor; ctx.font = `bold ${textSize}px ${fontFamily}`; ctx.fillText(part, currentX, y); currentX += ctx.measureText(part).width;
                } else {
                  ctx.fillStyle = textColor; ctx.font = `${textSize}px ${fontFamily}`; ctx.fillText(part, currentX, y); currentX += ctx.measureText(part).width;
                }
              });
            }
            else { ctx.fillStyle = textColor; ctx.font = `${textSize}px ${fontFamily}`; ctx.fillText(line || "", x, y); }
          });
          ctx.fillStyle = mutedColor; ctx.font = `12px ${uiFont}`; ctx.textAlign = "center"; let footerText = `Generated with Sonata Master`; if (payload.creator) footerText += `  •  Arranged by: ${payload.creator}`; if (payload.links?.length) footerText += `  •  Ref: ${payload.links[0].url}`; ctx.fillText(footerText, width / 2, height - 24);
          const byteString = atob(canvas.toDataURL("image/png").split(",")[1]); const bytes = new Uint8Array(byteString.length); for (let i = 0; i < byteString.length; i++) bytes[i] = byteString.charCodeAt(i); return new Blob([bytes], { type: "image/png" });
        },
        ...
        createPdf(payloads) {
          ...
          pages.forEach((pageOps, index) => {
            ...
            let chrome = `q 0.96 0.96 0.98 rg 0 ${headerY} ${pageWidth} ${headerHeight} re f Q\n` +
              ...
              `0 g BT /F3 18 Tf ${margin + 36} ${pageHeight - 32} Td (${this.pdfEscape(this.pdfTruncate(p.title, 55))}) Tj ET\n`;
            if (p.artist) {
              chrome += `0.38 0.40 0.45 rg BT /F2 10.5 Tf ${margin + 36} ${pageHeight - 50} Td (By: ${this.pdfEscape(this.pdfTruncate(p.artist, 60))}) Tj ET\n` +
        ...
        async share(isSetlist = false) {
          ...
          try {
            if (rawUrl.length > 200) {
              ...
              formBody.append('content', await this.compressData(shareData));
              ...
              if (dpasteRes.ok) {
                const dpasteUrl = (await dpasteRes.text()).trim();
                const pasteId = dpasteUrl.substring(dpasteUrl.lastIndexOf('/') + 1);
                finalUrl = window.location.origin + window.location.pathname + "?paste=" + pasteId;
              } else {
                throw new Error("dpaste failed");
              }
            } else {
              const res = await fetch(`https://is.gd/create.php?format=json&url=${encodeURIComponent(rawUrl)}`);
              if (res.ok) { const data = await res.json(); if (data.shorturl) finalUrl = data.shorturl; }
            }
          } catch (e) {
            console.log("Shorteners unavailable, using full url");
          }
          ...
          const urlInput = document.createElement('input'); urlInput.className = 'input'; urlInput.readOnly = true; urlInput.value = finalUrl; urlInput.style.cssText = "margin-bottom:12px; font-size: 0.85rem; text-align:center; font-weight:600; color:var(--accent);";
          urlInput.addEventListener('click', () => urlInput.select());

          const nativeShare = async () => { try { if (navigator.share) await navigator.share({ title: titleText, text: `Check out this ${isSet ? 'Setlist' : 'Song'}: "${titleText}"`, url: finalUrl }); else UIManager.toast("Native sharing not supported on this device/browser."); } catch (e) { } };
          ...
          const noteText = document.createElement('p'); noteText.style.cssText = "font-size:0.75rem; color:var(--muted); text-align:center; margin:4px 0 0;"; noteText.textContent = "Data is securely compressed into this short link. Works entirely offline.";
      ...
      const UIManager = {
        ...
        bind() {
          ...
          add(this.dom.saveSongButton, "click", () => { AudioEngine.playClick(); StateManager.saveNow("Saved"); this.toast("Song saved to Library"); });
          add(this.dom.undoSongButton, "click", () => { AudioEngine.playClick(); Editor.undo(); this.toast("Undo"); });
          add(this.dom.redoSongButton, "click", () => { AudioEngine.playClick(); Editor.redo(); this.toast("Redo"); });
          add(this.dom.importSharedButton, "click", () => { AudioEngine.playClick(); StateManager.importShared(); Editor.loadActiveSong(); this.renderAll(); this.toast("Saved to Library"); });
          ...
          add(this.dom.loadDemoButton, "click", () => {
            ...
            this.openModal({
              title: "Load Demo",
              message: "Load demo song? This will replace your current song content.",
              confirmText: "Load Demo",
              onConfirm: () => {
                StateManager.loadDemo();
                Editor.loadActiveSong();
                this.renderAll();
                this.toast("Demo song loaded");
              }
            });
          });
          ...
          add(this.dom.librarySortSelect, "change", () => { this.renderLibrarySoon(); this.toast("Sorted: " + this.dom.librarySortSelect.options[this.dom.librarySortSelect.selectedIndex].text); });
          document.querySelectorAll("[data-library-filter]").forEach(b => add(b, "click", () => { AudioEngine.playClick(); StateManager.state.libraryFilter = b.dataset.libraryFilter; document.querySelectorAll("[data-library-filter]").forEach(i => i.classList.toggle("active", i === b)); this.renderLibrarySoon(); this.toast("Filter: " + b.textContent); }));
          add(this.dom.newSongButton, "click", () => { AudioEngine.playClick(); if (StateManager.state.libraryFilter === 'setlists') { this.openSetlistModal(); return; } StateManager.createSong(); Editor.loadActiveSong(); this.switchView('editor'); this.renderAll(); this.toast("New song ready"); });
          ...
          add(this.dom.applyTransposeButton, "click", () => { AudioEngine.playClick(); const song = StateManager.activeSong(); if (!song || song.readonly) return; const delta = StateManager.state.transposeDelta; if (!delta) { this.toast("No transpose selected"); return; } this.openModal({ title: "Apply Transpose", message: "Permanently rewrite chords in editor?", confirmText: "Apply", onConfirm: () => { song.body = TransposeEngine.transposeText(song.body, delta, KeyDetector.activeKey(song, StateManager.state.detectedKey || KeyDetector.detect(song.body))); StateManager.state.transposeDelta = 0; if (this.dom.transposeSelect) this.dom.transposeSelect.value = "0"; Editor.loadActiveSong(); StateManager.touch(song); StateManager.saveNow("Transposed"); this.renderAll(); this.toast("Chords transposed permanently"); } }); });
          add(this.dom.resetTransposeButton, "click", () => { AudioEngine.playClick(); this.openModal({ title: "Reset Modifications", message: "Reset Capo and Transpose?", confirmText: "Reset", onConfirm: () => { StateManager.state.transposeDelta = 0; this.dom.transposeSelect.value = "0"; StateManager.state.capo = 0; this.dom.capoSelect.value = "0"; this.updateAnalysis(); this.toast("Reset"); } }); });
        ...
        renderAll() { const song = StateManager.activeSong(); if (this.dom.keySelect) this.dom.keySelect.value = song?.manualKey || "auto"; if (this.dom.transposeSelect) this.dom.transposeSelect.value = String(StateManager.state.transposeDelta); if (this.dom.capoSelect) this.dom.capoSelect.value = String(StateManager.state.capo); MetronomeManager.renderBeats(); MetronomeManager.updateUi(); this.updateScrollControls(); this.updateSetlistNav(); this.updateAnalysis(); InstrumentManager.renderCircle(); this.renderLibrarySoon(); if (this.dom.theoryKeyInput) this.dom.theoryKeyInput.value = KeyDetector.activeKey(song, StateManager.state.detectedKey)?.name || ""; },
        updateAnalysis() {
        ...
        openManageLibrary() {
          ...
          bulkShareBtn?.addEventListener('click', async () => {
            ...
            const rawUrl = window.location.origin + window.location.pathname + "?share=" + compressed;
            let finalUrl = rawUrl;
            ...
            try {
              ...
              console.warn("Shortener failed, using full URL");
            }
        ...
        openSettings() {
          ...
          this.openModal({
            title: "Settings", confirmText: "Save Settings", fields: [
              { type: "heading", label: "Appearance" }, { id: "theme", label: "Theme", type: "select", value: s.theme, options: [{ value: "light", label: "Light" }, { value: "dark", label: "Dark" }] }, { id: "accentTheme", label: "Color theme", type: "select", value: s.accentTheme, options: ACCENT_THEMES }, { id: "uiFontFamily", label: "Interface font", type: "select", value: s.uiFontFamily, options: UI_FONT_OPTIONS }, { id: "chartFontFamily", label: "Song chart font", type: "select", value: s.chartFontFamily, options: CHART_FONT_OPTIONS }, { id: "appFontSize", label: "Interface text size", type: "range", value: s.appFontSize, min: 10, max: 24, step: 1, suffix: "px" }, { id: "editorFontSize", label: "Editor text size", type: "range", value: s.editorFontSize, min: 8, max: 64, step: 1, suffix: "px" }, { id: "previewFontSize", label: "Preview text size", type: "range", value: s.previewFontSize, min: 8, max: 48, step: 1, suffix: "px" }, { id: "editorLineHeight", label: "Song line spacing", type: "range", value: s.editorLineHeight, min: 1.2, max: 2.8, step: 0.02, suffix: "x" },
              { type: "heading", label: "Music & Interactivity" }, { id: "metronomeVolume", label: "Metronome Tick Volume", type: "range", value: Math.round(s.metronomeVolume * 100), min: 0, max: 100, step: 5, suffix: "%" }, { id: "uiSounds", label: "UI Sounds", type: "select", value: s.uiSounds ? "true" : "false", options: [{ value: "true", label: "On" }, { value: "false", label: "Off" }] }, { id: "haptics", label: "Haptic Feedback", type: "select", value: s.haptics ? "true" : "false", options: [{ value: "true", label: "On" }, { value: "false", label: "Off" }] },
              { type: "heading", label: "Data & Backup" }, { type: "custom", html: `<div style="display:flex;gap:8px;flex-wrap:wrap;"><button class="button" onclick="const d = { songs: StateManager.state.songs, setlists: StateManager.state.setlists, settings: StateManager.state.settings }; Util.download('sonata-backup.json', 'application/json', JSON.stringify(d, null, 2)); UIManager.toast('Backup exported');" type="button">Export Backup</button><label class="button" style="margin:0;cursor:pointer;"><input type="file" accept=".json" style="display:none;" onchange="const r = new FileReader(); r.onload = (e) => { try { const d = JSON.parse(e.target.result); if (d.songs) StorageManager.saveSongs(d.songs); if (d.setlists) StorageManager.saveSetlists(d.setlists); if (d.settings) StorageManager.saveSettings(d.settings); UIManager.toast('Backup restored!'); setTimeout(() => window.location.reload(), 1000); } catch(err) { UIManager.toast('Invalid backup file'); } }; if(this.files.length) r.readAsText(this.files[0]);">Import Backup</label></div>` },
              { type: "heading", label: "Google Drive Sync" }, { type: "custom", html: '<div id="settingsDriveSyncStatus" style="padding: 4px 0;"></div>' },
              { id: "installAppContainer", type: "custom", html: `<button class="button primary" id="installAppBtn" style="width:100%;display:none;" type="button">Install App to Device</button><p id="iosInstallTip" style="margin:6px 0 0;font-size:0.75rem;color:var(--muted);display:none;">To install on iOS/Mac: Tap Share then "Add to Home Screen".</p>` },
              { type: "custom", html: `<button class="button danger" onclick="if(confirm('Reset all appearance settings?')) { StateManager.state.settings = Object.assign({}, DEFAULT_SETTINGS); StateManager.saveNow('Settings restored'); window.location.reload(); }" type="button" style="width:100%; margin-top: 14px;">Restore Default Settings</button>` }
            ], onConfirm: v => { s.theme = v.theme; s.accentTheme = v.accentTheme; s.uiFontFamily = v.uiFontFamily; s.chartFontFamily = v.chartFontFamily; s.appFontSize = v.appFontSize; s.editorFontSize = v.editorFontSize; s.previewFontSize = v.previewFontSize; s.editorLineHeight = v.editorLineHeight; s.metronomeVolume = v.metronomeVolume / 100; s.uiSounds = v.uiSounds === "true"; s.haptics = v.haptics === "true"; ThemeManager.apply(); Editor.applySettings(); PresentationManager.applySettings(); MetronomeManager.updateUi(); StateManager.saveNow("Settings saved"); this.toast("Settings saved"); }
          });
      ...
      const ThemeManager = { apply() { const theme = StateManager.state.settings.theme; document.documentElement.dataset.theme = theme; document.documentElement.dataset.accent = StateManager.state.settings.accentTheme || "blue"; if (UIManager.dom.themeToggle) Icon.set(UIManager.dom.themeToggle, theme === "dark" ? "sun" : "moon", theme === "dark" ? "Light Mode" : "Dark Mode", true); } };
        ...
        render() {
            ...
            state.setlists.forEach(set => {
              ...
              card.addEventListener('click', (e) => {
                ...
                if (actionBtn) {
                  ...
                  if (action === 'delete') { UIManager.openModal({ title: "Delete Setlist", message: "Delete this setlist?", confirmText: "Delete", destructive: true, onConfirm: () => { StateManager.state.setlists = StateManager.state.setlists.filter(s => s.id !== set.id); StateManager.saveNow("Setlist Deleted"); if (state.activeSetlist.id === set.id) StateManager.exitSetlist(); this.render(); UIManager.toast("Setlist Deleted"); } }); }
          ...
          songs.forEach(song => {
            ...
            card.addEventListener('click', (e) => {
              ...
              if (actionBtn) {
                e.stopPropagation(); const action = actionBtn.dataset.action;
                if (action === 'fav') { song.isFavorite = !song.isFavorite; StateManager.saveNow("Favorite Toggled"); this.render(); UIManager.toast(song.isFavorite ? "Added to Favorites" : "Removed from Favorites"); }
                if (action === 'delete') { StateManager.setActive(song.id); UIManager.openModal({ title: "Delete Song", message: "Delete this song?", confirmText: "Delete", destructive: true, onConfirm: () => { StateManager.deleteActive(); Editor.loadActiveSong(); UIManager.renderAll(); UIManager.toast("Deleted"); } }); }
      ...
      const App = {
        ...
        async init() {
          try {
            ...
            if ('serviceWorker' in navigator) navigator.serviceWorker.register(URL.createObjectURL(new Blob(["const CACHE = 'sonata-v3'; self.addEventListener('install', e => { self.skipWaiting(); e.waitUntil(caches.open(CACHE).then(c => c.addAll([location.pathname]))); }); self.addEventListener('activate', e => { e.waitUntil(caches.keys().then(keys => Promise.all(keys.map(k => k !== CACHE ? caches.delete(k) : null)))); }); self.addEventListener('fetch', e => { e.respondWith(fetch(e.request).then(res => { const clone = res.clone(); caches.open(CACHE).then(c => c.put(e.request, clone)); return res; }).catch(() => caches.match(e.request))); });"], { type: 'application/javascript' }))).catch(() => { });
            ...
            if (shareParam) {
              try {
                const d = JSON.parse(await ExportManager.decompressData(shareParam));
                await this.handleImport(d);
                // Show preview modal with shared content details
                const isSet = d.type === 'set';
                const title = d.t || '';
                const subtitle = !isSet && d.a ? `By: ${d.a}` : '';
                const arranger = d.cr ? `Arranged by: ${d.cr}` : '';
                const links = (d.l || []).map(l => `<a href="${l.url}" target="_blank" style="color:var(--accent);">${l.name}</a>`).join(' &middot; ');
                const description = d.d || '';
                const bodyHtml = isSet ? `<p>${description}</p>` : `<pre style="white-space:pre-wrap;">${d.b}</pre>`;
                const previewHtml = `
                  <div style="font-family:var(--ui-font); color:var(--text);">
                    <h2 style="margin:0 0 8px;">${title}</h2>
                    ${subtitle ? `<p style="margin:0 0 4px;">${subtitle}</p>` : ''}
                    ${arranger ? `<p style="margin:0 0 4px;">${arranger}</p>` : ''}
                    ${links ? `<p style="margin:0 0 8px;">${links}</p>` : ''}
              ...
              try {
                ...
                if (resp.ok) {
                  const rawBase64 = await resp.text();
                  const d = JSON.parse(await ExportManager.decompressData(rawBase64.trim()));
                  await this.handleImport(d);
                }
              } catch (e) { }
            }
