...
`;
      ...
      const GoogleDriveSync = {
        DEFAULT_CLIENT_ID: "158653989463-6dicjrekd70pp2mh39scp05skr6ilcl6.apps.googleusercontent.com",
        ...
        getClientId() {
          const custom = localStorage.getItem('sonata_google_client_id');
          return (custom && custom.trim()) ? custom.trim() : this.DEFAULT_CLIENT_ID;
        },

        setClientId(newId) {
          if (newId && newId.trim()) {
            localStorage.setItem('sonata_google_client_id', newId.trim());
          } else {
            localStorage.removeItem('sonata_google_client_id');
          }
                ...
                callback: (tokenResponse) => {
                  if (tokenResponse.error !== undefined) {
                    ...
                    if (tokenResponse.error === "origin_mismatch" || tokenResponse.error_description?.includes("origin_mismatch")) {
                      UIManager.openModal({
                        title: "Google OAuth Origin Setup Required",
                        confirmText: "Got It",
                        fields: [
                          {
                            type: "custom",
                            html: `
                              <div style="font-size:0.88rem; line-height:1.5; color:var(--text); display:flex; flex-direction:column; gap:10px;">
                                <p style="margin:0;">Google OAuth requires registering the active domain in <strong>Google Cloud Console</strong>.</p>
                                <div style="background:var(--surface-2); padding:10px 12px; border-radius:6px; border:1px solid var(--line);">
                                  <div style="font-size:0.75rem; color:var(--muted); margin-bottom:4px;">Current Domain Origin:</div>
                                  <code style="font-size:0.85rem; font-weight:700; color:var(--accent); word-break:break-all;">${window.location.origin}</code>
                                </div>
                                <ol style="padding-left:18px; margin:0; font-size:0.82rem; color:var(--muted); display:flex; flex-direction:column; gap:6px;">
                                  <li>Go to <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener" style="color:var(--accent); text-decoration:underline;">Google Cloud Console &rarr; Credentials</a></li>
                                  <li>Open your <strong>OAuth 2.0 Client ID</strong></li>
                                  <li>Add <strong><code>${window.location.origin}</code></strong> to <em>Authorized JavaScript origins</em></li>
          ...
          if (this.accessToken) {
            ...
            container.innerHTML = `
              <div style="display:flex; flex-direction:column; gap:10px;">
                <p style="font-size:0.85rem; color:var(--muted); margin:0;">Backup and sync your library to Google Drive seamlessly.</p>
                <button class="button primary" id="settingsSignInBtn" type="button" style="width:100%;">Sign in with Google</button>
                <div style="margin-top:6px; padding:8px 10px; background:var(--surface-2); border:1px solid var(--line); border-radius:6px;">
                  <div style="font-size:0.75rem; color:var(--muted); display:flex; justify-content:space-between; align-items:center;">
                    <span>Active Domain Origin:</span>
                    <strong style="color:var(--accent);">${window.location.origin}</strong>
                  </div>
                </div>
              </div>
            `;
      ...
      const UIManager = {
        ...
        switchView(viewName) {
          ...
          if (viewName === "about") {
            const cl = document.getElementById('aboutChangelogContent');
            if (cl && cl.textContent.includes('Loading')) {
                fetch('update_log.json').then(r => r.json()).then(logs => {
                  cl.innerHTML = logs.map(v => `<details style="margin-bottom:6px;border:1px solid var(--line);border-radius:6px;overflow:hidden;"><summary style="padding:10px 14px;cursor:pointer;font-weight:700;font-size:0.95rem;background:var(--surface-2);color:var(--text);display:flex;justify-content:space-between;list-style:none;"><span>v${v.version}</span><span style="color:var(--muted);font-weight:400;">${v.date}</span></summary><p style="font-style:italic;color:var(--accent);margin:12px 16px 8px;font-size:0.9rem;">${v.whatsNew||''}</p><ul style="padding:0 16px 16px 32px;margin:0;line-height:1.8;font-size:0.9rem;">${(v.changes||[]).map(c=>`<li>${c}</li>`).join('')}</ul></details>`).join('');
                }).catch(() => { cl.textContent = 'No release notes available offline.'; });
            }
          }
